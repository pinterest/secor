...
- **[Parquet] Files (for JSON messages)**: JSON messages can be written to Parquet files. In addition to setting all options necessary for Protobuf messages, the JSON topics must be explicitly defined using the option `secor.topic.message.format.<topic>=JSON` or `secor.topic.message.format.*=JSON` if all Kafka topics use JSON. 
...
- **[Parquet] Files (for Avro messages)**: Columnar storage format. To use this output format, set `secor.file.reader.writer.factory=com.pinterest.secor.io.impl.AvroParquetFileReaderWriterFactory` option. In addition, thrift message class per Kafka topic must be defined using option `secor.thrift.message.class.<topic>=<thrift class name>`. The `schema.registry.url` option must be set. 
