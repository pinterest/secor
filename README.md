...
- **[Parquet] Files (for JSON messages)**: Columnar storage format. In addition to setting all options necessary to write Protobuf to Parquet, the JSON topics must be explicitly defined using the option `secor.topic.message.format.<topic>=JSON` or `secor.topic.message.format.*=JSON` if all Kafka topics use JSON. The protobuf classes defined per topic will be used as intermediaries between the JSON messages and Parquet files.
