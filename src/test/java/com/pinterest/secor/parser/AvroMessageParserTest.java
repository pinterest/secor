...
public class AvroMessageParserTest {
    ...
    public void setUp() throws Exception {
        ...
        Mockito.when(config.getSchemaRegistryUrl()).thenReturn("http://localhost:8081");
        ...
        encoder.flush();
        messageWithTimestamp = new Message("test-topic", 0, 0, EMPTY_BYTES, baos1.toByteArray());

        encoder = EncoderFactory.get().blockingBinaryEncoder(baos2, encoder);
        datumWriter.write(user2, encoder);
        encoder.flush();
        messageWithoutTimestamp = new Message("test-topic", 0, 0, EMPTY_BYTES, baos2.toByteArray());

        encoder = EncoderFactory.get().blockingBinaryEncoder(baos3, encoder);
        datumWriter.write(user3, encoder);
        encoder.flush();
        messageWithBadTimestamp = new Message("test-topic", 0, 0, EMPTY_BYTES, baos3.toByteArray());
