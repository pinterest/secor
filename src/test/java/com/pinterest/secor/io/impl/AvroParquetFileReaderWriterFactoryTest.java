...
public class AvroParquetFileReaderWriterFactoryTest extends TestCase {
    ...
    public void setUp() throws Exception {
        ...
        when(secorSchemaRegistryClient.decodeMessage("test-avro-topic",
                                                     AvroParquetFileReaderWriterFactory.serializeAvroRecord(writer,
                                                                                                            msg1))).thenReturn(
                msg1);
        when(secorSchemaRegistryClient.decodeMessage("test-avro-topic",
                                                     AvroParquetFileReaderWriterFactory.serializeAvroRecord(writer,
    ...
    public void testAvroParquetReadWriteRoundTrip() throws Exception {
        when(config.getFileReaderWriterFactory()).thenReturn(AvroParquetFileReaderWriterFactory.class.getName());

        LogFilePath tempLogFilePath =
                new LogFilePath(Files.createTempDir().toString(), "test-avro-topic", new String[]{"part-1"}, 0, 1,
                                23232, ".avro");
