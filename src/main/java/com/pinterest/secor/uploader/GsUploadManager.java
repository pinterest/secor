 ...
 * <p>
 * It will use Service Account credential (json file) that can be generated from the Google Developers Console. By
 * default it will look up configured credential path in secor.gs.credentials.path or fallback to the default credential
 * in the environment variable GOOGLE_APPLICATION_CREDENTIALS.
 * <p>
 * Application credentials documentation https://developers.google
 * .com/identity/protocols/application-default-credentials
 *
...
public class GsUploadManager extends UploadManager {
    ...
    /**
     * Global instance of the Storage. The best practice is to make it a single globally shared instance across your
     * application.
     */
    ...
    public GsUploadManager(SecorConfig config) throws Exception {
        ...
        mClient = getService(mConfig.getGsCredentialsPath(), mConfig.getGsConnectTimeoutInMs(),
                             mConfig.getGsReadTimeoutInMs());
    }

    private static Storage getService(String credentialsPath, int connectTimeoutMs, int readTimeoutMs)
            throws Exception {
        if (mStorageService == null) {
            ...
            // These retrySettings are copied from default com.google.cloud
            // .ServiceOptions#getDefaultRetrySettingsBuilder and changed initial retry delay to 5 seconds from 1
            // seconds.
            ...
            mStorageService = builder.build().getService();
        }
    ...
    public Handle<?> upload(LogFilePath localPath) throws Exception {
        ...
        final BlobInfo sourceBlob = BlobInfo.newBuilder(BlobId.of(gsBucket, gsKey))
            ...
            public void run() {
                try {
                    if (directUpload) {
                        ...
                        try (WriteChannel out = mClient.writer(sourceBlob); FileChannel in = new FileInputStream(
                                localFile).getChannel();) {
                            ByteBuffer buffer = ByteBuffer.allocateDirect(
                                    1024 * 1024 * 5); // 5 MiB buffer (remember this is pr. thread)
                        ...
                        LOG.debug("Upload file {} to gs://{}/{} in {} msec", localFile, gsBucket, gsKey,
