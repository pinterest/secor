...
/**
 * FileRegistry keeps track of local log files currently being appended to and
...
public class FileRegistry {
	 ...
	 *            The topic partition to retrieve paths for.
	...
	public Collection<LogFilePath> getPaths(TopicPartition topicPartition) {
		...
		return Collections.unmodifiableSet(logFilePaths);
	}

	/**
	 * Retrieve a writer for a given path or create a new one if it does not
	 * exist.
	 * 
	 * @param path
	 *            The path to retrieve writer for.
	 * @return Writer for a given path.
	 * @throws IOException
	 */
	public Writer getOrCreateWriter(final StorageFactory wFactory,
			LogFilePath path) throws IOException {
	...
	public void deleteTopicPartition(TopicPartition topicPartition)
			throws IOException {
		HashSet<LogFilePath> paths = mFiles.get(topicPartition);
		if (paths == null) {
			return;
	...
	/**
	 * Delete writer for a given topic partition. Underlying file is not
	 * removed.
	 * 
	 * @param path
	 *            The path to remove the writer for.
	 */
	...
	/**
	 * Delete all writers in a given topic partition. Underlying files are not
	 * removed.
	 * 
	 * @param topicPartition
	 *            The topic partition to remove the writers for.
	 */
	public void deleteWriters(TopicPartition topicPartition) throws IOException {
		HashSet<LogFilePath> paths = mFiles.get(topicPartition);
		if (paths == null) {
			LOG.warn("No paths found for topic " + topicPartition.getTopic()
