...
public class VectorColumnFiller {
    ...
    public static void fillRow(int rowIndex, JsonConverter[] converters, TypeDescription schema,
                               VectorizedRowBatch batch, JsonObject data) {
        ...
        public void convert(JsonElement value, ColumnVector vect, int row) {
            if (value == null || value.isJsonNull()) {
                ...
                for (int c = 0; c < obj.size(); ++c) {
                    childrenConverter.convert(obj.get(c), vector.child, (int) vector.offsets[row] + c);
                }
            }
        }
    }
