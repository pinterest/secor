...
public class ReflectionUtil {
     ...
     * <p>
     * The class passed in by name must be assignable to UploadManager and have 1-parameter constructor accepting a
     * SecorConfig.
     * <p>
    ...
    public static UploadManager createUploadManager(String className, SecorConfig config) throws Exception {
        ...
        if (!UploadManager.class.isAssignableFrom(clazz)) {
            throw new IllegalArgumentException(String.format("The class '%s' is not assignable to '%s'.", className,
     ...
     * <p>
     * The class passed in by name must be assignable to Uploader. See the secor.upload.class config option.
     *
    ...
    public static Uploader createUploader(String className) throws Exception {
        ...
        if (!Uploader.class.isAssignableFrom(clazz)) {
            throw new IllegalArgumentException(
                    String.format("The class '%s' is not assignable to '%s'.", className, Uploader.class.getName()));
        }
    ...
    /**
     * Create a MessageParser from it's fully qualified class name. The class passed in by name must be assignable to
     * MessageParser and have 1-parameter constructor accepting a SecorConfig. Allows the MessageParser to be pluggable
    ...
    /**
     * Create a FileReaderWriterFactory that is able to read and write a specific type of output log file. The class
     * passed in by name must be assignable to FileReaderWriterFactory. Allows for pluggable FileReader and FileWriter
     * instances to be constructed for a particular type of log file.
     * <p>
    ...
    private static FileReaderWriterFactory createFileReaderWriterFactory(String className, SecorConfig config)
        ...
        try {
            // Try to load constructor that accepts single parameter - secor configuration instance
     ...
     * @throws Exception on error
     */
    public static FileWriter createFileWriter(String className, LogFilePath logFilePath, CompressionCodec codec,
     ...
     * @throws Exception on error
     */
    public static FileReader createFileReader(String className, LogFilePath logFilePath, CompressionCodec codec,
                                              SecorConfig config) throws Exception {
        return createFileReaderWriterFactory(className, config).BuildFileReader(logFilePath, codec);
    }

    /**
     * Create a MessageTransformer from it's fully qualified class name. The class passed in by name must be assignable
     * to MessageTransformers and have 1-parameter constructor accepting a SecorConfig. Allows the MessageTransformers
     * to be pluggable by providing the class name of a desired MessageTransformers in config.
     * <p>
     ...
     * @throws Exception on error
     */
    public static MessageTransformer createMessageTransformer(String className, SecorConfig config) throws Exception {
        Class<?> clazz = Class.forName(className);
        if (!MessageTransformer.class.isAssignableFrom(clazz)) {
            throw new IllegalArgumentException(String.format("The class '%s' is not assignable to '%s'.", className,
                                                             MessageTransformer.class.getName()));
        }
        return (MessageTransformer) clazz.getConstructor(SecorConfig.class).newInstance(config);
    }

    /**
     * Create an MetricCollector from its fully qualified class name.
     * <p>
     * The class passed in by name must be assignable to MetricCollector. See the secor.monitoring.metrics.collector
     * .class config option.
     *
     ...
     *                                a primitive type, or void; or if the class has no nullary constructor; or if the
    ...
    /**
     * Create a ORCSchemaProvider from it's fully qualified class name. The class passed in by name must be assignable
     * to ORCSchemaProvider and have 1-parameter constructor accepting a SecorConfig. Allows the ORCSchemaProvider to be
     * pluggable by providing the class name of a desired ORCSchemaProvider in config.
     * <p>
     ...
     * @throws Exception on error
     */
    public static ORCSchemaProvider createORCSchemaProvider(String className, SecorConfig config) throws Exception {
        Class<?> clazz = Class.forName(className);
        if (!ORCSchemaProvider.class.isAssignableFrom(clazz)) {
            throw new IllegalArgumentException(String.format("The class '%s' is not assignable to '%s'.", className,
                                                             ORCSchemaProvider.class.getName()));
        }
        return (ORCSchemaProvider) clazz.getConstructor(SecorConfig.class).newInstance(config);
    }
}
